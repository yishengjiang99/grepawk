...
const xfs = {
    ...
    list_fs_graph_table:async  function(parent_node, ws){
        var dbrows =await db.query("select * from fs_graph where parent_node = $1",[parent_node]);
    ...
    list_files_table: async (pwd, ws) => {
        try {
            ...
            var headers = ['title', 'parent', 'size', 'type', 'last_modified', 'opts'];
            azfiles.forEach((file) => {
                ...
                rows.push({
                    ...
                    'opts': [{
                        cmd: `edit ${url} ${containerName + "/" + file.name}`,
                        desc: "edit"
                    }]
                })
            })
    ...
    init_fs_graph: async function () {
        ...
        db.insertTable("fs_graph", root).then(parent => {
