...
exports.new_user = async function (uuid,username) {
    try {
        var user = await exports.query("insert into users (uuid, xp, points, cwd,username) values ($1, $2, $3, $4,$5) returning *",
            [uuid, 0, 0, '',username]);
        return user[0];
    } catch (err) {
        console.log(err);
