      ...
      function _cmd_string(cmd_str) {
        ...
        switch (cmd) {
          case 'watch':
            ...
            var iframeHTML = '<iframe width="560" height="315" src="https://www.youtube.com/embed/' +
              args[0] + '?rel=0" frameborder="0" allowfullscreen></iframe>';
            outputHtml(iframeHTML);
            break;
          case 'cam':
            var iframeHTML =
              '<iframe width="560" height="315" src="/cam.html" frameborder="0" allowfullscreen></iframe>';
            outputHtml(iframeHTML);
            break;
          case 'download':
          ...
          case 'new':
            ...
            break;
          case 'upload':
            var formId =`file-${generateUUID()}`;
            var inputField = `<input type='file' id='${formId}' name='file' />`;
            outputHtml(inputField);
            VfsClient.uploader(formId);
            break;
          default:
      ...
      return {
        init: async function () {
          ...
          socket.onmessage = function (event) {
            if (event.data.startsWith("stdout: ")) {
