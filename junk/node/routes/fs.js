...
const base_path = path.dirname(path.dirname(dirname));
...
var walkSync = function(dir, filelist) {
  ...
  files.forEach(function(file) {
    ...
    if (stat.isDirectory()) {
      ...
      filelist.push({'path':dir,'is_dir':true, 'stat':stat,'size':stat.size,'filename':file,'created':new Date(stat.birthtimeMs),'updated':new Date(stat.atimeMs)});
    }
    else {
