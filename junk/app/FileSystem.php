...
class FileSystem extends Model {
            ...
            if($this->current_node->storage_type==='psql_table'){
                ...
                foreach($list as $child) {
                    ...
                    if ($is_folder) {
                        ...
                        $rows[] = ['cmd' => "convert $name psql", "mimetype"=>$mimeType, 'type' => $mimeType, 'display' => "Convert file into psql", 'link' => "onclick:cat ".urlencode($name)];
                    }
                    foreach($rows as &$row){
                        if(isset($row['link'])){
                            $cmd=$row['cmd'];
    ...
    public function cat($filename) {
        ...
        if (strpos($mimetype, "image") !== false) {
            ...
            $output = "<b>$filename</b>";
            $output .= "<br><br>";
            $output .= "<p><pre style='color:white'>".Storage::get($filepath)."</pre></p>";
